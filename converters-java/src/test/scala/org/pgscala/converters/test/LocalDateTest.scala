...
//      res.toString() should equal(SQLdate)
      ...
      TestDb.using{ db =>
        val date = db.row("select 'now'::date"){rS => rS.get[String](1)}.get
        given(""" a starting String value for sql date "%s""" format date)
        when("that value is converted to LocalDate")
        val res = LocalDate.parse(date)
        then ("It should return a LocalDate value %s" format LocalDate.now())
        res should equal(LocalDate.now())
      }
    }
...
//      res.toString() should equal(SQLdate)
      ...
      TestDb.using{ db =>
        val date = db.row("select 'epoch'::date"){rS => rS.get[String](1)}.get
        given(""" a starting String value for sql date "%s"""" format date)
        when("that value is converted to LocalDate")
        val res = LocalDate.parse(date)
        then ("It should return a LocalDate value 1970-01-01")
        res should equal(LocalDate.parse("1970-01-01"))
      }
    }
...
//      res.toString() should equal(SQLdate)
      TestDb.using{ db =>
        val date = db.row("select 'today'::date"){rS => rS.get[String](1)}.get
        given(""" a starting String value for sql date "%s"""" format date)
        when("that value is converted to LocalDate")
        val res = LocalDate.parse(date)
        then ("It should return a LocalDate value %s" format LocalDate.now())
...
//      res.toString() should equal(timestamp)
      TestDb.using{ db =>
        val date = db.row("select 'tomorrow'::date"){rS => rS.get[String](1)}.get
        given(""" a starting String value for sql date "%s"""" format date)
        when("that value is converted to LocalDate")
        val res = LocalDate.parse(date)
        then ("It should return a LocalDate value %s" format LocalDate.now().plusDays(1))
        res should equal(LocalDate.now().plusDays(1))
      }
    }

    scenario("POSTGRESQL: LocalDate to String Nr. 1"){
      TestDb.using{ db =>
        val date = db.row("select 'now'::date"){rS => rS.get[LocalDate](1)}.get
        given(""" a starting LocalDate value for sql date %s"""" format date)
        when("that value is converted to String")
        val res = date.toString
        then ("It should return a String value %s" format LocalDate.now.toString)
        res should equal(LocalDate.now().toString)
      }
    }

    scenario("POSTGRESQL: LocalDate to String Nr. 2"){
      TestDb.using{ db =>
        val date = db.row("select 'epoch'::date"){rS => rS.get[LocalDate](1)}.get
        given(""" a starting LocalDate value for sql date %s"""" format date)
        when("that value is converted to String")
        val res = date.toString
        then ("It should return a String value %s" format LocalDate.now.toString)
        res should equal("1970-01-01")
      }
    }

    scenario("POSTGRESQL: LocalDate to String Nr. 3"){
      TestDb.using{ db =>
        val date = db.row("select 'tomorrow'::date"){rS => rS.get[LocalDate](1)}.get
        given(""" a starting LocalDate value for sql date %s"""" format date)
        when("that value is converted to String")
        val res = date.toString
        then ("It should return a String value %s" format LocalDate.now().plusDays(1).toString)
        res should equal(LocalDate.now().plusDays(1).toString)
      }
    }
